...
Section~\ref{sec:withinretail} of the main paper summarizes the IV-decile decomposition of the within-retail wrong-signed literacy gradient on the persistent measure (five positive deciles, five negative; observation-weighted average $+0.0002$). This section reports the full table with within-decile state and permno counts.
...
For completeness, the PIT-literacy version of the mid-IO concentration table is reported here. Section~\ref{sec:mech-bounds} of the main paper engages the magnitude collapse.
...
Table~\ref{tab:prepost} of the main paper reports the pre-2015 and post-2015 split (Panels A and B). This section reports the full five-window grid for the headline Fama-MacBeth estimator on the static-snapshot full panel.
...
Section~\ref{sec:robust-eiv} of the main paper reports the firm-month-weighted reliability ratio $\lambda = 0.420$ and the EIV-corrected mid-IO coefficient. Table~\ref{tab:eiv_ia} reports the per-wave reliability ratios.
